...
function hasGitChanges() {
  try {
    ...
    if (error.message.includes('dubious ownership')) {
      console.log('🔐 检测到 Git 所有权问题，自动修复...');
      try {
        ensureGitSafety();
        // 重试一次
        const status = execSync('git status --porcelain', { encoding: 'utf8' });
        ...
        console.error('❌ Git 所有权修复失败:', retryError.message);
        return false;
      }
    }
    console.error('❌ Git 检查错误:', error.message);
    return false;
  }
}
...
function getGitChanges() {
  try {
    ...
    if (error.message.includes('dubious ownership')) {
      try {
        ensureGitSafety();
        return getGitChanges(); // 递归重试
      } catch (retryError) {
        return '无法获取更改详情（所有权问题）';
      }
    }
    return '无法获取更改详情';
  }
...
function commitAllChangesToGitHub(commitMessage) {
  try {
    ...
    if (error.message.includes('Permission denied') || error.message.includes('Authentication failed')) {
      ...
      console.log('2. 将公钥添加到GitHub: cat ~/.ssh/id_ed25519.pub');
      console.log('3. 测试连接: ssh -T git@github.com');
      console.log('4. 更改远程地址为SSH: git remote set-url origin git@github.com:username/repository.git');
    }
